...
<body>
	<%
		...
		User user = new UserDBProcess().getUser(userId);
			...
			int result = userProc.userUpdate(userId, request.getParameter("userPassword"),
					request.getParameter("userEmail"), request.getParameter("userName"));
			if (result == -1) { 
				PrintWriter script = response.getWriter();
				...
				PrintWriter script = response.getWriter();
				script.println("<script>alert('회원정보를 수정하였습니다.')</script>");
				script.println("<script>location.href = 'login.jsp'</script>");
			}
	%>
</body>
